...
type Daemon struct {
	...
	// NodeLayeredUpdaterClient wraps rpm-ostree and will eventually be removed with a direct rpmostreeclient value
	NodeLayeredUpdaterClient *RpmOstreeClient

	NodeContainerizedUpdaterClient *BootcClient
...
func New(
	...
	var NodeLayeredUpdaterClient *RpmOstreeClient

	// Only pull the osImageURL from OSTree when we are on RHCOS or FCOS
	if hostos.IsCoreOSVariant() {
		NodeLayeredUpdaterClientVal := NewNodeLayeredUpdaterClient()
		NodeLayeredUpdaterClient = &NodeLayeredUpdaterClientVal
		err := NodeLayeredUpdaterClient.Initialize()
		if err != nil {
			return nil, fmt.Errorf("error initializing rpm-ostree: %w", err)
		}
		osImageURL, osVersion, osCommit, err = NodeLayeredUpdaterClient.GetBootedOSImageURL()
		if err != nil {
	...
	return &Daemon{
		...
		rebootQueued:             false,
		os:                       hostos,
		NodeLayeredUpdaterClient: NodeLayeredUpdaterClient,
...
func (dn *Daemon) getStateAndConfigs() (*stateAndConfigs, error) {
	...
	// fail.
	if dn.NodeLayeredUpdaterClient != nil {
		finalizeError, err := dn.NodeLayeredUpdaterClient.Peel().QueryPreviousDeploymentError()
		if err != nil {
...
func newMachineConfigDiff(oldConfig, newConfig *mcfgv1.MachineConfig) (*machineConfigDiff, update.Updater, error) {
	// this is where we should establish the new updater. So that later on we can just do `.update`
	oldIgn, err := ctrlcommon.ParseAndConvertConfig(oldConfig.Spec.Config.Raw)
	if err != nil {
		return nil, nil, fmt.Errorf("parsing old Ignition config failed with error: %w", err)
	}
	newIgn, err := ctrlcommon.ParseAndConvertConfig(newConfig.Spec.Config.Raw)
	if err != nil {
		return nil, nil, fmt.Errorf("parsing new Ignition config failed with error: %w", err)
	}
	...
	if oldConfig.Spec.OSImageURL != newConfig.Spec.OSImageURL {
		// determine what kind of image, and add something to mcDiff to tell us we if a) we are swapping image types and b) what type we are using
		isLayeredImage, err := IsBootableImage(newConfig.Spec.OSImageURL)
		if err != nil {
			return nil, nil, fmt.Errorf("Error checking type of update image: %w", err)
		}

		// so if we need to do an "update" this is the highest point at which it is called. I think here we should shell out to the update functions which means
		// we need to establish our update type far in advance, particularly when mcDiff.osUpdate is resolved.
		if isLayeredImage {
			// init layered updater and handler
			updater = update.NewLayeredUpdater()
		} else {
			updater = update.NewContainerizedUpdater()
			// init bootable updater and handler
		}
