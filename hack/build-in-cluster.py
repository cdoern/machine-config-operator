    ...
    return {
        ...
        "spec": {
            "output": {
                "to": {
                    "kind": "ImageStreamTag",
                    "name": get_imagestream_spec()["metadata"]["name"] + ":latest",
                }
            },
            "postCommit": {},
...
def start_mco_image_build():
    ...
    apply_object(get_imagestream_spec())
    ...
    return build["status"]["outputDockerImageReference"].replace(":latest", "@"+digest)

# Scales down both the cluster version operator and the machine config operator
# to prepare for building the cluster image.
def prepare_for_build():
    # Scale down the cluster version operator to ensure that it does not
    # overwrite our image updates.
...
def can_run():
    ...
    for binary in binaries:
        if not shutil.which(binary):
            print("Did not find required binary '%s'" % binary)
            return False

    # If we can't determine the git remote, fail.
    git_remote = get_git_remote()
    if not git_remote:
        print("No forked git remote found. Either add one or set %s and try again!" % GIT_FORK_URL)
        return False
